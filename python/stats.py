...
def bar(ratio, width=20, high_is_good=True):
    ...
    return _c(color, "\u2588" * filled) + _c("2", "\u2591" * empty)
...
def _load_all(provider_filter: Optional[str] = None) -> list[Session]:
    ...
    if provider_filter:
        ...
        if not p:
            print(_c("31", f"\u2717 Unknown provider '{provider_filter}'"))
            print(f"  Available: {', '.join(p.name for p in all_providers())}")
            sys.exit(1)
        if not p.detect():
            print(_c("31", f"\u2717 Provider '{provider_filter}' has no data on this machine"))
            sys.exit(1)
        providers = [p]
...
def cmd_list(all_sessions: list[Session], provider_filter: Optional[str] = None):
    ...
    for i, s in enumerate(all_sessions, 1):
        print(
            f"  {_c('2', f'{str(i):>4}')} "
            f"{_c('36', f'{s.id[:24]:<24}')} "
            f"{_c('33', f'{s.provider:<10}')} "
            f"{_c('97', f'{(s.title or "")[:48]:<50}')} "
...
def cmd_analyze(session: Session, all_sessions: Optional[list[Session]] = None):
    ...
    if all_sessions and len(all_sessions) >= 5:
        ...
        if z_total > 2:
            anomalies.append((_c("31", "\u26a0"), f"Anomalously large ({z_total:.1f}\u03c3 above median)"))
        elif z_total > 1.5:
            anomalies.append((_c("33", "\u26a0"), f"Larger than average ({z_total:.1f}\u03c3 above median)"))
        elif z_total < -1.5:
            anomalies.append((_c("32", "\u2713"), f"Smaller than average ({abs(z_total):.1f}\u03c3 below median)"))
        ...
        if cost > 0:
            ...
            if z_cost > 2:
                anomalies.append((_c("31", "\u26a0"), f"Anomalously expensive ({z_cost:.1f}\u03c3 above median)"))

        this_ratio = to / max(this_total, 1)
        m_ratio = median(raw_ratios)
        if this_ratio > m_ratio * 1.5:
            anomalies.append((_c("33", "\u26a0"), f"Output ratio higher than typical"))
        elif this_ratio < m_ratio * 0.5:
            anomalies.append((_c("33", "\u26a0"), f"Input ratio higher than typical"))
    ...
    if n_steps > 1:
        ...
        if n_steps >= 5:
            ...
            print(f"  Distribution:  {_c('32', f'\u25a0 {s_pct:.0f}%')} {_c('33', f'\u25a0 {m_pct:.0f}%')}"
                  f" {_c('31', f'\u25a0 {l_pct:.0f}%')}  "
    ...
    if total_savable > 0:
        ...
        if cache_savings:
            print(f"  {_c('32', '\u2191')}  Cache:        +{format_num(cache_savings):>8}  "
                  f"({_c('2', 'improve cache hit rate')})")
        if step_savings:
            print(f"  {_c('33', '\u2191')}  Batching:     +{format_num(step_savings):>8}  "
                  f"({_c('2', 'fewer steps = less overhead')})")
        if reasoning_savings:
            print(f"  {_c('33', '\u2191')}  Reasoning:    +{format_num(reasoning_savings):>8}  "
                  f"({_c('2', 'lower reasoningEffort')})")
    else:
        print(f"  {_c('32', 'Session is already optimal \u2713')}")
    ...
    if tips:
        ...
        for severity, title, items in tips:
            icon = _c("31", "\u25cf") if severity == "critical" else (
                _c("33", "\u25cf") if severity == "warning" else _c("32", "\u25cf"))
...
def cmd_outliers(all_sessions: list[Session]):
    ...
    for z, label, s in found[:15]:
        if s.id in seen_ids:
            continue
        seen_ids.add(s.id)
        print(f"  {_c('31', '\u25b6')} {_c('1', f'{label:<50}')}  {_c('36', f'{s.id[:20]:<20}')}  "
...
def cmd_compare(s1: Session, s2: Session):
    ...
    if t1 < t2 and c1 <= c2:
        ...
        print(f"{_c('33', 'Trade-off \u2014 depends on priority (speed vs cost)')}")
    print()
...
def cmd_trends(all_sessions: list[Session], days: int = 30):
    ...
    for key in dates:
        ...
        bar_s = _c(color, "\u2588" * filled) + _c("2", "\u2591" * (barlen - filled))
    ...
    print(f"\n  {_c('2', f'{active}/{days} days active  \u00b7  {total_s} sessions  \u00b7  {format_num(total_t)} total tokens')}")
    print()
...
def cmd_budget(all_sessions: list[Session], args: list[str]):
    ...
    if monthly_limit > 0:
        ...
        bar_s = _c(color, "\u2588" * filled) + _c("2", "\u2591" * (barw - filled))
        ...
        if remaining > 0:
            ...
            print(f"  {_c('31', f'  \u26a0 Exceeded by ${abs(remaining):.4f}')}")
    else:
        print(f"  Spent:        ${month_spend_usd:.4f}")
...
def cmd_search(all_sessions: list[Session], query: str):
    ...
    if not matching:
        print(_c("31", f"\u2717 No sessions matching '{query}'"))
        return []

    print(f"\n  {_c('1', f'Sessions matching \u201c{query}\u201d')}  (found: {len(matching)})\n")
...
def main():
    ...
    if not detect_providers() and args and args[0] not in no_data_commands:
        print(_c("31", "\u2717 No supported coding agents detected."))
    ...
    if not args:
        ...
        if len(args) < 2:
            print(_c("31", "\u2717 Usage: tokenstats analyze <N|id|last>"))
            sys.exit(1)
        s = _resolve_session(all_sessions, args[1])
        if not s:
            print(_c("31", f"\u2717 Session '{args[1]}' not found."))
            sys.exit(1)
        ...
        if not s1 or not s2:
            print(_c("31", f"\u2717 Session not found. Use numbers or IDs."))
            sys.exit(1)
        cmd_compare(s1, s2)
        ...
        if not s:
            print(_c("31", f"\u2717 Session '{args[0]}' not found."))
